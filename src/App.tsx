...
const App: React.FC = () => {
  ...
  const [activeLiveSubTab, setActiveLiveSubTab] =
    useState<SubVistaVivo>("grupos_vivo");
  const [selectedStadium, setSelectedStadium] = useState<InfoEstadio | null>(
    null
  );
  ...
  const [selectedEliminatoriasFases, setSelectedEliminatoriasFases] = useState<
    Fase[]
  >(todasFasesEliminatorias);
      ...
      try {
        ...
        if (res.success) {
          ...
          if (res.data.globalSettings) {
            ...
            if (
              gs.timeConfig?.modoTiempo === "SIMULACION" &&
              gs.timeConfig.fechaSimulacion
            ) {
  ...
  const mejoresTerceros = useMemo(
    () => obtenerMejoresTerceros(tablaGeneral),
    [tablaGeneral]
  );
  ...
  const mejoresTercerosOficiales = useMemo(
    () => obtenerMejoresTerceros(tablaOficial),
    [tablaOficial]
  );
  ...
  const cargarLeaderboard = useCallback(async () => {
    setIsLoading(true);
    try {
      // Solo leer scores ya calculados del backend
      ...
      toast("Error al cargar ranking", "error");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRandomGroups = () => {
    if (
      globalSettings.groupsPhaseLocked ||
      appTime >= new Date(DEADLINE_GROUPS)
    ) {
  ...
  const globalValidation = useMemo(() => {
    const allMatches = [...PARTIDOS_INICIALES, ...llaves].filter(
      (m) => !isMatchLocked(m)
    );
    const incomplete = allMatches.filter(
      (m) =>
        !isValidScore(pronosticos[m.id]?.local) ||
        !isValidScore(pronosticos[m.id]?.visitante)
    );
    return {
  ...
  const handleSaveProgress = async (freeze = false) => {
    ...
    for (let i = 1; i <= 104; i++) {
      ...
      sanitizedPronos[i] = {
        local:
          p.local !== null && p.local !== "-" && p.local !== ""
            ? String(p.local)
            : "-",
        visitante:
          p.visitante !== null && p.visitante !== "-" && p.visitante !== ""
            ? String(p.visitante)
            : "-",
        penalesLocal:
          p.penalesLocal !== null &&
          p.penalesLocal !== "-" &&
          p.penalesLocal !== ""
            ? String(p.penalesLocal)
            : "-",
        penalesVisitante:
          p.penalesVisitante !== null &&
          p.penalesVisitante !== "-" &&
          p.penalesVisitante !== ""
            ? String(p.penalesVisitante)
            : "-",
      };
    }
    try {
      const res = await api.savePredictions(
        user!.id,
        user!.username,
        sanitizedPronos,
        freeze
  ...
  const handleAuthSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isRegistering) {
      ...
      if (res.success) {
        ...
        if (
          loggedUser.activo === false ||
          String(loggedUser.activo) === "false"
        ) {
        ...
        <div className="w-full max-w-md bg-zinc-900 p-8 sm:p-10 rounded-[2.5rem] border border-zinc-800 text-center space-y-8 shadow-2xl animate-in fade-in zoom-in duration-500">
          ...
          <form onSubmit={handleAuthSubmit} className="space-y-4 text-left">
            {isRegistering && (
              <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-top-2">
                <input
                  ...
                  placeholder="Nombre"
                  required
                  className="bg-zinc-800 border-zinc-700 rounded-xl p-3 text-xs text-white outline-none focus:ring-1 focus:ring-emerald-500"
                />
                <input
                  name="apellido"
            ...
            {isRegistering && (
              <input
                ...
                required
                className="w-full bg-zinc-800 border-zinc-700 rounded-xl p-3 text-xs text-white outline-none focus:ring-1 focus:ring-emerald-500"
              />
            )}
            <input
              name="u"
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full bg-zinc-800 border-zinc-700 rounded-xl p-3 text-xs text-white outline-none focus:ring-1 focus:ring-emerald-500 pr-10"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-600 hover:text-emerald-500"
              >
                {showPassword ? (
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-emerald-500 p-4 rounded-xl font-black uppercase text-xs tracking-widest text-zinc-950 hover:bg-emerald-400 active:scale-95 transition-all shadow-lg flex items-center justify-center gap-2"
            >
              {isLoading ? (
          ...
          <div className="space-y-4 pt-4 border-t border-zinc-800/50 text-center">
            <button
              ...
              className="text-[10px] font-black uppercase text-emerald-500 hover:text-emerald-400 tracking-widest"
            >
              {isRegistering
      ...
      <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans overflow-x-hidden">
        ...
        <header className="sticky top-0 z-[60] bg-zinc-950/80 backdrop-blur-xl border-b border-zinc-900 h-[64px] flex items-center px-6 no-print">
          <div className="max-w-7xl mx-auto w-full flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-black italic uppercase text-lg tracking-tighter">
                Mundial 2026
              </span>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="p-2 bg-zinc-900 border border-zinc-800 rounded-xl text-emerald-500 hover:bg-zinc-800 transition-colors"
              >
                <Menu className="w-5 h-5" />
              </button>
              <button
                onClick={handleLogout}
                className="bg-rose-500/10 hover:bg-rose-500 px-3 py-2 rounded-xl border border-rose-500/20 text-rose-500 hover:text-white font-black text-[10px] uppercase transition-all"
              >
                Salir
              </button>
            </div>
        ...
        <div className="bg-zinc-900/40 border-b border-zinc-900 px-6 py-4 no-print">
          <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-6">
            <CountdownTimer
              currentTime={appTime}
              targetDate={DEADLINE_GROUPS}
              label="Fase de Grupos"
              isManuallyOverridden={globalSettings.groupsPhaseLocked}
            />
            <div className="h-8 w-[1px] bg-zinc-800 hidden md:block"></div>
            <CountdownTimer
              currentTime={appTime}
              targetDate={DEADLINE_KNOCKOUT}
              label="Fase Clasificatoria"
              isManuallyOverridden={globalSettings.knockoutPhaseLocked}
            />
            {isTimeManual && (
              <div className="ml-auto flex items-center gap-2 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-500">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest">
                  Simulación de Tiempo Activa (24H)
                </span>
              </div>
            )}
          </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex gap-2">
                <button
                  ...
                  className="p-3 bg-zinc-900 border border-zinc-800 rounded-xl hover:bg-zinc-800 text-zinc-400"
                >
                  <Printer className="w-4 h-4" />
                </button>
                <button
                  onClick={() => setShowManual(true)}
                  className="flex items-center gap-2 px-4 py-3 bg-zinc-900 border border-zinc-800 text-zinc-400 rounded-xl font-black text-[10px] uppercase"
                >
            ...
            {activeTab !== "vivo" && !isFrozen && (
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="bg-zinc-800 border border-zinc-700 text-zinc-300 px-6 py-3 rounded-xl font-black text-[10px] uppercase hover:bg-zinc-700 transition-all"
                >
                  Guardar Borrador
                </button>
                <button
                  onClick={() => handleSaveProgress(true)}
                  disabled={!globalValidation.isValid}
                  className={`bg-emerald-500 text-zinc-950 px-6 py-3 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all ${
            ...
            {isFrozen && (
              <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 px-6 py-3 rounded-xl">
                ...
                <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest italic">
                  Pronóstico Finalizado y Protegido
                </span>
              </div>
            )}
          </div>
          ...
          <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col gap-8">
                  ...
                  <div className="flex flex-wrap gap-2 no-print bg-zinc-900/50 p-1.5 rounded-2xl border border-zinc-800 self-start">
                    {[
                      ...
                      {
                        id: "clasificacion_vivo",
                        label: "Clasificación",
                        icon: BarChart3,
                      },
                      {
                        id: "eliminatorias_vivo",
                        label: "Eliminatorias",
                        icon: Zap,
                      },
                      ...
                      <button
                        key={sub.id}
                        onClick={() =>
                          setActiveLiveSubTab(sub.id as SubVistaVivo)
                        }
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${
                          activeLiveSubTab === sub.id
                            ? "bg-zinc-800 text-emerald-400 border border-emerald-500/20 shadow-lg"
                            : "text-zinc-500 hover:text-white"
                        }`}
                      >
                    ...
                    {GRUPOS.map((g) => (
                      <div key={g} className="space-y-4">
                        ...
                        <div className="space-y-3">
                          {PARTIDOS_INICIALES.filter((m) => m.grupo === g).map(
                            (m) => (
                              <MatchCard
                                key={m.id}
                                match={m}
                  ...
                  <div className="max-w-3xl mx-auto space-y-6">
                    <div className="bg-zinc-900/50 p-10 rounded-[3rem] border border-zinc-800 shadow-2xl relative overflow-hidden">
                      <div className="absolute right-0 top-0 p-8 opacity-10">
                        <Flag className="w-32 h-32 text-emerald-500" />
                      </div>
                      <h3 className="text-2xl font-black uppercase italic text-white mb-8 tracking-tighter">
                        Tabla de Mejores Terceros (Top 8)
                      </h3>
                            ...
                            {mejoresTercerosOficiales.map((t, idx) => {
                              ...
                              return (
                                <tr
                                  key={t.equipoId}
                                  className={`${
                                    avanza ? "bg-emerald-500/5" : "opacity-40"
                                ...
                                >
                                  ...
                                  <td
                                    className={`px-4 py-5 text-center font-black ${
                                      t.dg > 0
                                        ? "text-emerald-500"
                                        : "text-zinc-500"
                                    }`}
                                  ...
                                  <td className="px-4 py-5 text-center font-bold text-zinc-600">
                                    {t.gf}
                                  </td>
                                </tr>
                              );
                      ...
                      <div className="mt-8 flex items-center gap-3 text-zinc-500">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest italic">
                          Los mejores 8 avanzan a Dieciseisavos por criterios
                          oficiales de desempate.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {activeLiveSubTab === "eliminatorias_vivo" && (
                  <div className="space-y-16">
                    {todasFasesEliminatorias.map((fase) => {
                      const matchesByFase = llavesOficiales.filter(
                        (m) => m.fase === fase
                      );
            ...
            {activeTab === "grupos" && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {GRUPOS.map((g) => (
                  <div key={g} className="space-y-4">
                    <h3 className="text-xl font-black text-white italic uppercase border-b border-zinc-900 pb-2">
                      Grupo {g}
                    </h3>
                    <div className="space-y-3">
                      {PARTIDOS_INICIALES.filter((m) => m.grupo === g).map(
                ...
                <div className="flex flex-col gap-6 no-print">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-black uppercase tracking-widest italic text-white">
                      Filtrar Tramos de Eliminación
                    </h3>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() =>
                        setSelectedEliminatoriasFases(todasFasesEliminatorias)
                      }
                      className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                        selectedEliminatoriasFases.length ===
                        todasFasesEliminatorias.length
                    ...
                    {todasFasesEliminatorias.map((fase) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                          selectedEliminatoriasFases.includes(fase)
                            ? "bg-emerald-500/20 text-emerald-500 border-emerald-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)]"
                            : "bg-zinc-900 text-zinc-500 border-zinc-800 hover:text-white"
                        }`}
                      >
                ...
                {selectedEliminatoriasFases.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-32 space-y-6 bg-zinc-900/10 rounded-[4rem] border-2 border-dashed border-zinc-800">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-zinc-600 italic">
                      Selecciona un tramo para visualizar los partidos
                    </p>
                  </div>
                )}
              </div>
        ...
        {showToast && (
          <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[150] animate-in slide-in-from-bottom-5">
            <div
              className={`px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest border shadow-2xl ${
