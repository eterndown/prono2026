 ...
 */
export const obtenerMejoresTerceros = (
  todas: Record<LetraGrupo, Clasificacion[]>
): Clasificacion[] => {
...
export const resolverLlaves = (
  ...
  const fuentePosiciones =
    usarDatosReales && datosReales ? datosReales.posiciones : posiciones;
  const fuenteTerceros =
    usarDatosReales && datosReales ? datosReales.terceros : terceros;
  ...
  const getWinner = (id: number, loc: string | null, vis: string | null) => {
    ...
    if (
      !p ||
      !isValidScore(p.local) ||
      !isValidScore(p.visitante) ||
      !loc ||
      !vis
    )
      return null;
  ...
  > = {
    ...
    78: {
      d: "Martes 30 Junio",
      h: "18:00",
      e: "Nueva York Nueva Jersey Stadium",
      pl: "1I",
      pv: "3C/D/F/G/H",
      lId: get1("I"),
      vId: getT(1),
    ...
    82: {
      d: "Miércoles 1 Julio",
      h: "21:00",
      e: "San Francisco Bay Area Stadium",
      pl: "1D",
      pv: "3B/E/F/I/J",
      lId: get1("D"),
      vId: getT(5),
  ...
  for (let id = 73; id <= 88; id++) {
    ...
    r32.push({
      id,
      fase: "Dieciseisavos",
      equipoLocal: meta.lId,
      equipoVisitante: meta.vId,
      placeholderLocal: meta.pl,
      placeholderVisitante: meta.pv,
      golesLocal: pronosticos[id]?.local ?? null,
      golesVisitante: pronosticos[id]?.visitante ?? null,
      penalesLocal: pronosticos[id]?.penalesLocal ?? null,
      penalesVisitante: pronosticos[id]?.penalesVisitante ?? null,
      fecha: meta.d + " 2026",
      hSimu: meta.h,
      estadio: meta.e,
    });
  ...
  const r16Map: Record<
    number,
    { d: string; h: string; e: string; l: number; v: number }
  > = {
    89: { d: "Sábado 4 Julio", h: "14:00", e: "Houston Stadium", l: 73, v: 75 },
    90: {
      d: "Sábado 4 Julio",
      h: "18:00",
      e: "Philadelphia Stadium",
      l: 74,
      v: 77,
    },
    91: {
  ...
  for (let id = 89; id <= 96; id++) {
    ...
    const locId = getWinner(
      meta.l,
      lMatch?.equipoLocal || null,
      lMatch?.equipoVisitante || null
    );
    const visId = getWinner(
      meta.v,
      vMatch?.equipoLocal || null,
      vMatch?.equipoVisitante || null
    );
    r16.push({
      id,
      fase: "Octavos",
      equipoLocal: locId,
      equipoVisitante: visId,
      placeholderLocal: `W${meta.l}`,
      placeholderVisitante: `W${meta.v}`,
      golesLocal: pronosticos[id]?.local ?? null,
      golesVisitante: pronosticos[id]?.visitante ?? null,
      penalesLocal: pronosticos[id]?.penalesLocal ?? null,
      penalesVisitante: pronosticos[id]?.penalesVisitante ?? null,
  ...
  const t3 = {
    id: 103,
    fase: "TercerPuesto" as Fase,
    equipoLocal: getLoser(
      101,
      semis[0]?.equipoLocal || null,
      semis[0]?.equipoVisitante || null
    ),
    equipoVisitante: getLoser(
      102,
      semis[1]?.equipoLocal || null,
      semis[1]?.equipoVisitante || null
    ),
    placeholderLocal: "L101",
    placeholderVisitante: "L102",
    golesLocal: pronosticos[103]?.local ?? null,
    golesVisitante: pronosticos[103]?.visitante ?? null,
    penalesLocal: pronosticos[103]?.penalesLocal ?? null,
    penalesVisitante: pronosticos[103]?.penalesVisitante ?? null,
  ...
  const fin = {
    id: 104,
    fase: "Final" as Fase,
    equipoLocal: getWinner(
      101,
      semis[0]?.equipoLocal || null,
      semis[0]?.equipoVisitante || null
    ),
    equipoVisitante: getWinner(
      102,
      semis[1]?.equipoLocal || null,
      semis[1]?.equipoVisitante || null
    ),
    placeholderLocal: "W101",
    placeholderVisitante: "W102",
    golesLocal: pronosticos[104]?.local ?? null,
    golesVisitante: pronosticos[104]?.visitante ?? null,
    penalesLocal: pronosticos[104]?.penalesLocal ?? null,
    penalesVisitante: pronosticos[104]?.penalesVisitante ?? null,
    fecha: "Domingo 19 Julio 2026",
    hSimu: "16:00",
    estadio: "Nueva York Nueva Jersey Stadium",
  };
