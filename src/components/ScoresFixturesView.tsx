...
const ScoresFixturesView: React.FC<ScoresFixturesViewProps> = ({
  pronosticos,
  eliminatorias,
  onUpdate,
  disabled,
  ...
  const [hoveredStadiumMatch, setHoveredStadiumMatch] = useState<number | null>(
    null
  );
  ...
  const filteredAndSortedMatches = useMemo(() => {
    ...
    let filtered = combined.filter((m) => {
      ...
      const matchEquipo =
        filters.equipo === "all" ||
        m.equipoLocal === filters.equipo ||
        m.equipoVisitante === filters.equipo;
  ...
  const handleScoreChange = (
    ...
    if (value.trim() === "") processed = null;
      ...
      processed = Math.min(Math.max(0, parseInt(value, 10)), 15);
    else return;
    ...
    <div className="max-w-6xl mx-auto space-y-12 pb-32 animate-in fade-in duration-700 px-4 sm:px-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8 border-b-2 border-zinc-900 pb-10">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 no-print w-full lg:w-auto">
          ...
          <div className="flex flex-col gap-2 w-full sm:w-64">
            <span className="text-[9px] font-black uppercase tracking-widest text-zinc-600 pl-1">
              Buscar partido:
            </span>
          ...
          <div className="flex flex-col gap-2 w-full sm:w-auto">
            ...
            <div className="flex bg-zinc-900 p-1.5 rounded-2xl border border-zinc-800 shadow-xl w-full sm:w-auto">
              <button
                ...
                className={`flex-1 sm:flex-none px-4 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${
                  sortBy === "date-asc"
      ...
      {filteredAndSortedMatches.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-40 space-y-8 bg-zinc-900/10 rounded-[4rem] border-2 border-dashed border-zinc-800/50">
          ...
          <h3 className="text-2xl font-black uppercase italic text-zinc-500 tracking-tighter">
            Sin coincidencias
          </h3>
          <button
            onClick={() => {
            ...
            <div key={date} className="space-y-8">
              ...
              <div className="grid grid-cols-1 gap-6">
                {matches.map((m) => {
                  ...
                  const isDraw =
                    isNumeric(pred.local) &&
                    isNumeric(pred.visitante) &&
                    parseInt(String(pred.local), 10) ===
                      parseInt(String(pred.visitante), 10);

                  const estadioInfo = Object.values(ESTADIOS).find(
                    (info) => info.e === m.estadio
                  );
                  ...
                  return (
                    <div
                      key={m.id}
                      className={`relative bg-zinc-900/30 hover:bg-zinc-900/50 border border-zinc-900 rounded-[2.5rem] p-6 sm:p-10 transition-all duration-500 group ${
                        isHoveredMatch ? "z-50" : "z-10"
                      }`}
                    >
                      <div className="flex flex-col lg:flex-row items-center justify-between gap-10 relative z-10">
                        ...
                        <div className="flex flex-col items-center gap-4 min-w-[200px] shrink-0">
                          ...
                          <div
                            className={`flex items-center gap-6 bg-zinc-950 px-8 py-5 rounded-3xl border border-zinc-800 shadow-2xl ${
                              disabled ? "opacity-80" : ""
                            }`}
                          >
                            <input
                              ...
                              onChange={(e) =>
                                handleScoreChange(m.id, "local", e.target.value)
                              }
                              className={`w-14 h-14 bg-zinc-900 border border-zinc-800/50 text-center rounded-2xl text-2xl font-black outline-none transition-all ${
                                disabled
                                  ? "text-zinc-600"
                                  : "text-white focus:ring-1 focus:ring-emerald-500 focus:bg-zinc-800"
                              }`}
                            />
                            ...
                            <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/20 px-6 py-2 rounded-2xl animate-in fade-in duration-300">
                              ...
                              <div className="flex gap-2">
                                <input
                                  ...
                                  className={`w-8 h-8 bg-zinc-900 text-center rounded-lg text-sm font-black text-emerald-500 outline-none ${
                                    disabled
                                      ? "opacity-50"
                        ...
                        <div className="flex flex-wrap items-center gap-6">
                          ...
                          >
                            <div
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                if (estadioInfo && onSelectStadium)
                                  onSelectStadium(estadioInfo);
                              }}
                            >
