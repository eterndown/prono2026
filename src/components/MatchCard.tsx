...
export const MatchCard: React.FC<MatchCardProps> = memo(
  ({ match, prediction, disabled, isReadOnly, onUpdate, onSelectStadium }) => {
    const local = EQUIPOS.find((t) => t.id === (match.equipoLocal || ""));
    const visitante = EQUIPOS.find(
      (t) => t.id === (match.equipoVisitante || "")
    );
    ...
    const showPens =
      esKO &&
      isNumericValue(prediction.local) &&
      isNumericValue(prediction.visitante) &&
      parseInt(String(prediction.local), 10) ===
        parseInt(String(prediction.visitante), 10);
    ...
    const handleScoreChange = (side: "local" | "visitante", value: string) => {
      ...
      if (trimmed === "") {
        onUpdate(
          match.id,
          side === "local" ? null : prediction.local,
          side === "visitante" ? null : prediction.visitante,
          prediction.penalesLocal,
          prediction.penalesVisitante
        );
        return;
      }
    ...
    const handlePensChange = (side: "local" | "visitante", value: string) => {
      ...
      if (trimmed === "") {
        onUpdate(
          ...
          prediction.local,
          prediction.visitante,
          side === "local" ? null : prediction.penalesLocal,
          side === "visitante" ? null : prediction.penalesVisitante
        );
        return;
      }
      const n = Number(trimmed);
    ...
    const stadiumInfo = Object.values(ESTADIOS).find(
      (s) => s.e === match.estadio
    );
    ...
    return (
      <div
        className={`bg-zinc-900/50 border-2 ${
          disabled && !isReadOnly
            ? "border-zinc-800/40 opacity-70 grayscale-[0.2]"
            : isReadOnly
            ? "border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.05)]"
            : "border-zinc-800 hover:border-zinc-700 shadow-xl"
        } p-4 rounded-3xl flex flex-col gap-4 transition-all duration-300 relative group/card ${
          prediction.esSimulacion && !isReadOnly
            ? "bg-amber-500/5 border-amber-500/30 shadow-[inset_0_0_20px_rgba(245,158,11,0.05)]"
            : ""
        }`}
      >
        {disabled && !isReadOnly && (
        ...
        {isReadOnly && (
          <div className="absolute -top-2 -right-2 flex items-center gap-2 bg-emerald-500 px-3 py-1 rounded-full shadow-lg z-20 animate-pulse">
            ...
            <span className="text-[8px] font-black uppercase text-zinc-950 tracking-widest">
              Oficial
            </span>
          </div>
        )}
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-2 w-full">
            <div className="flex items-center justify-between">
              <span
                className={`bg-zinc-950 border border-zinc-800 px-2.5 py-1 rounded-lg font-black text-[9px] uppercase tracking-tighter ${
                  isReadOnly
                    ? "text-emerald-400"
                    : prediction.esSimulacion
                    ? "text-amber-500"
                    : "text-emerald-500"
                }`}
              >
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 px-2 py-1 bg-zinc-950/60 border border-zinc-800 rounded-lg">
                ...
                <span className="text-[9px] font-black text-zinc-400 uppercase leading-none">
                  {match.fecha}
                </span>
              </div>
              <div className="flex items-center gap-1.5 px-2 py-1 bg-zinc-950/60 border border-zinc-800 rounded-lg">
                <Clock className="w-2.5 h-2.5 text-zinc-500" />
                <span className="text-[9px] font-black text-zinc-400 leading-none">
                  {match.hSimu || "16:00"} ARG
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between gap-3 px-2">
          ...
          >
            <div className="w-10 h-7 rounded-lg overflow-hidden border border-zinc-800 shadow-md transform group-hover/card:scale-105 transition-transform">
              <img
                ...
                alt={local?.nombre || "TBD"}
              />
            </div>
            <span
              className={`font-black truncate text-[10px] uppercase tracking-tighter w-full text-center ${
                disabled && !isReadOnly ? "text-zinc-500" : "text-zinc-300"
              }`}
            >
          ...
          <div
            className={`flex items-center gap-2 bg-zinc-950 p-2 rounded-2xl border-2 transition-colors ${
              disabled && !isReadOnly
                ? "border-zinc-800/30"
                : isReadOnly
          ...
          >
            <input
              ...
              className={`w-10 h-10 bg-zinc-900 border border-zinc-800/50 text-center rounded-xl text-sm font-black outline-none transition-all ${
                disabled && !isReadOnly
                  ? "text-zinc-700"
                  : isReadOnly
                  ? "text-emerald-400"
            ...
            />
            <span
              className={`font-black text-lg self-center ${
                disabled && !isReadOnly ? "text-zinc-800" : "text-zinc-700"
              }`}
            >
              :
            </span>
          ...
          <div
            className="flex-1 flex flex-col items-center gap-2 overflow-hidden"
            title={match.placeholderVisitante}
          >
            <div className="w-10 h-7 rounded-lg overflow-hidden border border-zinc-800 shadow-md transform group-hover/card:scale-105 transition-transform">
              <img
                src={getFlagUrl(visitante?.id || null)}
                className="w-full h-full object-cover"
                alt={visitante?.nombre || "TBD"}
              />
            </div>
            <span
              className={`font-black truncate text-[10px] uppercase tracking-tighter w-full text-center ${
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="P"
                  className={`w-7 h-7 bg-zinc-950 border text-center rounded-lg text-[10px] font-black outline-none transition-all ${
                    !pensValid
                ...
                <input
                  ...
                  onChange={(e) =>
                    handlePensChange("visitante", e.target.value)
                  }
                  placeholder="P"
                  className={`w-7 h-7 bg-zinc-950 border text-center rounded-lg text-[10px] font-black outline-none transition-all ${
                    !pensValid
        ...
        {match.estadio && (
          <button
            onClick={() =>
              stadiumInfo && onSelectStadium && onSelectStadium(stadiumInfo)
            }
