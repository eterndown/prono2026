...
const DetailedStandingsView: React.FC<DetailedStandingsViewProps> = ({
  tablaGeneral,
  isReadOnly,
}) => {
  return (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-[0.3em]">
            Resultados pronosticados por grupo • Fase de Grupos
          </p>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-zinc-900">
                {tablaGeneral[g].map((s, idx) => {
                  ...
                  return (
                    ...
                    >
                      <td
                        className={`py-5 pl-8 font-black text-sm ${
                          clasifica ? "text-emerald-500" : "text-zinc-700"
                        }`}
                      >
                        {idx + 1}
                      </td>
                      <td className="py-5 flex items-center gap-4">
                        ...
                        <span
                          className={`font-black text-xs uppercase tracking-tighter ${
                            clasifica ? "text-white" : "text-zinc-400"
                          }`}
                        >
                          {team?.nombre}
                        </span>
                      </td>
                      <td className="py-5 text-center font-bold text-xs text-zinc-500">
                        {s.pj}
                      </td>
                      <td className="py-5 text-center font-bold text-xs text-zinc-500">
                        {s.pg}
                      ...
                      <td
                        className={`py-5 text-center font-black text-xs ${
                          s.dg > 0
                            ? "text-emerald-500"
                            : s.dg < 0
                            ? "text-rose-500"
                      ...
                      <td
                        className={`py-5 text-center font-black text-sm ${
                          clasifica ? "text-emerald-400" : "text-white"
                        }`}
                      >
                        {s.puntos}
                      </td>
      ...
      <div className="mt-16 p-8 bg-zinc-900/30 rounded-[2.5rem] border border-zinc-900">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-8">
          {[
            ...
            <div key={idx} className="flex items-center gap-3">
              <span className="w-8 text-center font-black text-emerald-500 text-[10px] bg-zinc-950 py-1 rounded-md border border-zinc-800">
                {item.sigla}
              </span>
        ...
        <div className="mt-8 pt-6 border-t border-zinc-900 text-center">
          <p className="text-[8px] font-black text-zinc-700 uppercase tracking-[0.4em] italic">
            Criterios de desempate: 1. Puntos • 2. Diferencia de Goles • 3.
            Goles Marcados
          </p>
        </div>
      </div>
