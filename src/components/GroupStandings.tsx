    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] overflow-hidden shadow-2xl transition-all duration-300 hover:border-zinc-700">
      ...
      <div className="bg-gradient-to-r from-zinc-800/50 to-zinc-900/50 px-6 py-4 flex justify-between items-center border-b border-zinc-800">
        <div className="flex items-center gap-3">
          ...
          <h3 className="font-black text-white uppercase tracking-tighter italic text-lg">
            GRUPO {group}
          </h3>
        </div>
        <button
        ...
        <table className="w-full text-left text-[11px] border-collapse min-w-[500px]">
          <thead className="text-[9px] font-black uppercase text-zinc-500 bg-zinc-950/40 tracking-widest border-b border-zinc-800/50">
            <tr>
              ...
              {showForm && (
                <th className="px-4 py-3 text-center animate-in fade-in slide-in-from-right-2">
                  Forma
                </th>
              )}
            </tr>
              ...
              return (
                <tr
                  ...
                  className={`group transition-all duration-300 ${
                    clasifica ? "bg-emerald-500/5" : "hover:bg-zinc-800/20"
                  }`}
                >
                  <td
                    className={`px-4 py-3.5 font-black ${
                      clasifica ? "text-emerald-500" : "text-zinc-600"
                    }`}
                  >
                    {idx + 1}
                  </td>
                  <td className="px-4 py-3.5 font-black flex items-center gap-3">
                    ...
                    <span
                      className={`truncate uppercase tracking-tighter text-xs ${
                  ...
                  <td className="px-2 py-3.5 text-center font-bold text-zinc-500">
                    {s.pj}
                  </td>
                  <td className="px-2 py-3.5 text-center font-bold text-zinc-500">
                    {s.pg}
                  ...
                  <td
                    className={`px-2 py-3.5 text-center font-black ${
                      s.dg > 0
                        ? "text-emerald-500"
                        : s.dg < 0
                        ? "text-rose-500"
                  ...
                  <td
                    className={`px-4 py-3.5 text-center font-black text-sm ${
                      clasifica ? "text-emerald-400" : "text-zinc-200"
                    }`}
                  >
                    {s.puntos}
                  </td>
