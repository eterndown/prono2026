...
const StandingsSidebar: React.FC<StandingsSidebarProps> = ({
  ...
  return (
    <>
      ...
      {/* Sidebar Panel */}
      <aside
        className={`fixed top-0 right-0 h-full w-full max-w-[320px] bg-zinc-950 border-l border-zinc-800 z-[110] shadow-[0_0_50px_rgba(0,0,0,0.5)] transition-transform duration-500 ease-out transform ${
                    ...
                    {mejoresTerceros.slice(0, 8).map((t, idx) => {
                      ...
                      return (
                        <tr key={t.equipoId} className="hover:bg-emerald-500/5">
                          <td className="px-3 py-2 font-black text-emerald-500">
                            {idx + 1}
                          </td>
                          <td className="px-2 py-2 font-bold uppercase truncate max-w-[120px]">
                            <div className="flex items-center gap-2">
                              <img
                                ...
                                className="w-4 h-2.5 object-cover rounded-sm border border-zinc-800"
                              />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4">
                {GRUPOS.map((g) => (
                  <div
                    key={g}
                    className="space-y-2 bg-zinc-900/30 rounded-2xl p-3 border border-zinc-900"
                  >
                          ...
                          {tablaGeneral[g].map((s, idx) => {
                            ...
                            return (
                              <tr
                                key={s.equipoId}
                                className={
                                  idx < 2 ? "text-emerald-400" : "text-zinc-500"
                                }
                              >
                                <td className="py-1.5 font-black opacity-50 w-4">
                                  {idx + 1}
                                </td>
                                ...
                                <td className="py-1.5 text-right font-black w-8">
                                  {s.puntos}{" "}
                                  <span className="text-[7px] font-bold opacity-40 ml-1">
                                    PTS
                                  </span>
                                </td>
                              </tr>
