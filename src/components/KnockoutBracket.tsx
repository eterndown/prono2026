...
const BracketMatch: React.FC<{
  ...
  const isResolved =
    isNumeric(pronostico.local) && isNumeric(pronostico.visitante);
  ...
  const handleScoreChange = (side: "local" | "visitante", value: string) => {
    ...
    let processed: number | string | null =
      value === ""
        ? null
        : !isNaN(Number(value))
        ? Math.min(Math.max(0, Number(value)), 15)
        : value;
    if (side === "local")
      onUpdate(
        processed,
  ...
  return (
    <div
      className={`w-full bg-zinc-900 border ${
        disabled ? "border-amber-500/10" : "border-zinc-800"
    ...
    >
      ...
      <div className="px-2 pt-1.5 flex items-center justify-between border-b border-zinc-800/30">
        <div className="flex items-center gap-1.5">
          <span className="text-[8px] font-black text-emerald-500 uppercase px-1.5 py-0.5 bg-zinc-950 border border-zinc-800 rounded shadow-inner">
            M#{match.id}
          </span>
        ...
        {[
          ...
          <div
            ...
            className={`flex items-center justify-between p-1.5 rounded-lg transition-all duration-500 ${
              won
                ? "bg-emerald-500/20 ring-1 ring-emerald-500/30 shadow-lg"
                : isResolved && (visitorWon || localWon)
          ...
          >
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <span
                className={`text-[10px] font-black uppercase truncate tracking-tighter ${
                  won ? "text-emerald-400" : "text-zinc-400"
                }`}
              >
                {team?.nombre ||
                  match[`placeholder${side === "L" ? "Local" : "Visitante"}`] ||
                  "TBD"}
              </span>
            </div>
            <input
              ...
              className={`w-7 h-7 bg-zinc-950 border border-zinc-800 rounded text-center text-[10px] font-black outline-none transition-colors ${
                disabled
                  ? "text-zinc-600"
          ...
          <div className="px-3 pb-2 flex items-center justify-between gap-1 border-t border-zinc-800/20 pt-1.5">
            <span
              className={`text-[7px] font-black uppercase italic ${
                !pensValid ? "text-rose-500 animate-pulse" : "text-emerald-500"
              }`}
            >
              P:
            </span>
            <div className="flex gap-1">
              <input
                type="text"
                disabled={disabled}
                value={getDisplayValue(pronostico.penalesLocal)}
                onChange={(e) => handlePensChange("local", e.target.value)}
                className={`w-6 h-6 bg-zinc-950 border rounded text-center text-[9px] font-black outline-none transition-all ${
                  !pensValid
              ...
              <input
                type="text"
                disabled={disabled}
                value={getDisplayValue(pronostico.penalesVisitante)}
...
const KnockoutBracket: React.FC<KnockoutBracketProps> = ({
  ...
  const octavos = [89, 90, 91, 92, 93, 94, 95, 96].map(
    (id) => llaves.find((m) => m.id === id)!
  );
  const cuartos = [97, 98, 99, 100].map(
    (id) => llaves.find((m) => m.id === id)!
  );
    ...
    <div className="w-full overflow-x-auto pb-10 scrollbar-hide">
      <div className="min-w-[1000px] flex justify-between gap-4 p-4">
        <div className="flex flex-col justify-around gap-8 w-60">
          <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 mb-2 text-center">
            Octavos
          </h4>
          {octavos.map((m) => (
            <BracketMatch
        ...
        <div className="flex flex-col justify-around gap-16 w-60">
          <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500 mb-2 text-center">
            Cuartos
          </h4>
          {cuartos.map((m) => (
            <BracketMatch
        ...
        <div className="flex flex-col justify-around gap-32 w-60">
          <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-400 mb-2 text-center">
            Semis
          </h4>
          {semis.map((m) => (
            <BracketMatch
        ...
        <div className="flex flex-col justify-center gap-20 w-64">
          <div className="space-y-4">
            ...
            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white text-center">
              Gran Final
            </h4>
            <div
              className={`p-1 rounded-[1.5rem] bg-gradient-to-br from-emerald-500/10 border border-zinc-800 shadow-xl`}
            >
              <BracketMatch
                match={final}
                disabled={disabled}
                pronostico={
                  pronosticos[final.id] || { local: "-", visitante: "-" }
                }
                onUpdate={(l, v, pl, pv) => onUpdate(final.id, l, v, pl, pv)}
              />
            </div>
          </div>
          <div className="space-y-4">
            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 text-center">
              3er Puesto
            </h4>
            <BracketMatch
              match={tercerPuesto}
              disabled={disabled}
              pronostico={
                pronosticos[tercerPuesto.id] || { local: "-", visitante: "-" }
              }
              onUpdate={(l, v, pl, pv) =>
                onUpdate(tercerPuesto.id, l, v, pl, pv)
              }
            />
          </div>
        </div>
