...
interface Props {
  ...
  setRealScores: React.Dispatch<
    React.SetStateAction<Record<number, Pronostico>>
  >;
}
...
const AdminPanel: React.FC<Props> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<
    "usuarios" | "bloqueos" | "resultado_oficial" | "logs"
  >("usuarios");
  ...
  const fetchRealResults = async () => {
    ...
    try {
      ...
      toast("Error al cargar resultados reales", "error");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const updatePens = (id: number, side: "L" | "V", val: string) => {
    setRealScores((prev) => {
      ...
      return {
        ...
        [id]: {
          ...current,
          [side === "L" ? "penalesLocal" : "penalesVisitante"]:
            val === "" ? "-" : val,
        },
      };
    });
  ...
  const mejoresTercerosOficialAdmin = useMemo(
    () => obtenerMejoresTerceros(tablaOficialAdmin),
    [tablaOficialAdmin]
  );
  ...
  const saveRealResults = () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      toast(
        "Resultados actualizados localmente y sincronizando con el servidor...",
        "success"
      );
    }, 500);
  };
  ...
  const toggleUserStatus = async (targetId: string, currentStatus: boolean) => {
    setIsLoading(true);
    const res = await api.adminUpdateUser(currentUser.username, targetId, {
  ...
  const persistVirtualTime = async () => {
    ...
    try {
      ...
      if (res.success) {
        setAppTime(targetDate);
        toast("Fecha aplicada ✨");
      }
    } finally {
  ...
  const persistResetTime = async () => {
    ...
    try {
      ...
      if (res.success) {
        setIsTimeManual();
        toast("Tiempo real restablecido");
      }
    } finally {
      setIsSavingTime(false);
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest italic">
              Panel de Gobernanza Global • Formato 24H
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
            ...
            <div className="p-4 bg-emerald-500/10 rounded-3xl border border-emerald-500/20">
              <Clock
                className={`w-10 h-10 text-emerald-500 ${
                  !isTimeManual ? "animate-pulse" : ""
                }`}
              />
            </div>
            <div>
              <h4 className="text-xl font-black uppercase text-emerald-400 italic tracking-tighter leading-none mb-1">
                Reloj Maestro (24H)
              </h4>
              <div className="flex items-center gap-3">
                <span className="text-3xl font-mono font-black text-white tabular-nums tracking-tighter">
                  {formatMilitar(appTime)}
                </span>
                <div
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 bg-zinc-900 p-4 rounded-3xl border border-zinc-800 w-full md:w-auto">
            <div className="flex flex-col gap-1 w-full sm:w-auto">
              <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest pl-1">
                Ajustar Tiempo Virtual
              </span>
              <input
                type="datetime-local"
                ...
                lang="es-AR"
                step="60"
                className="bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs text-white outline-none font-mono focus:ring-1 focus:ring-emerald-500"
              />
            </div>
            <div className="flex gap-2 self-end">
              <button
                onClick={persistVirtualTime}
                disabled={isSavingTime}
                className="px-6 py-3 bg-emerald-500 text-zinc-950 rounded-xl hover:bg-emerald-400 font-black uppercase text-[10px] tracking-widest"
              >
                {isSavingTime ? (
              ...
              <button
                onClick={persistResetTime}
                disabled={isSavingTime}
                className="p-3 bg-zinc-800 text-zinc-400 rounded-xl border border-zinc-700"
              >
      ...
      <div className="bg-zinc-900/30 border border-zinc-800 rounded-[2.5rem] overflow-hidden min-h-[500px] flex flex-col">
                ...
                <tbody className="divide-y divide-zinc-800/30">
                  {filteredUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-emerald-500/5 group">
                      <td className="px-4 py-5 font-black text-xs text-white">
                        @{u.username}
                      </td>
                      ...
                      <td className="px-4 py-5 text-center">
                        {u.isFrozen ? (
                          ...
                          <Unlock className="w-3.5 h-3.5 text-zinc-700 mx-auto" />
                        )}
                      </td>
        ...
        {activeSubTab === "resultado_oficial" && (
          <div className="p-8 space-y-8 flex flex-col flex-1">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div>
                <h3 className="text-xl font-black uppercase italic text-white flex items-center gap-3">
                  <Wand2 className="w-5 h-5 text-emerald-500" /> Motor de
                  Clasificación Oficial
                </h3>
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mt-1">
                  Gestión de resultados del torneo real
                </p>
              </div>
              <div className="flex gap-3 w-full md:w-auto">
                <button
                  onClick={simulateAllReal}
                  className="flex-1 md:flex-none px-6 py-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl font-black text-[10px] uppercase hover:bg-amber-500/20"
                >
                  Sim
                </button>
                <button
                  onClick={clearAndSaveReal}
                  className="flex-1 md:flex-none px-6 py-3 bg-zinc-800 border border-zinc-700 text-zinc-500 rounded-xl font-black text-[10px] uppercase hover:text-rose-500 hover:border-rose-500/50 transition-all flex items-center justify-center gap-2"
                >
                  <Trash2 className="w-3.5 h-3.5" /> Vaciar
                </button>
                <button
                  onClick={saveRealResults}
                  disabled={isLoading}
                  className="flex-1 md:flex-none px-8 py-3 bg-emerald-500 text-zinc-950 rounded-xl font-black text-[10px] uppercase shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
              ...
              <table className="w-full text-left text-[11px] border-collapse min-w-[850px]">
                ...
                <tbody className="divide-y divide-zinc-800/30">
                  {filteredMatches.map((m) => {
                    ...
                    const isDraw =
                      isValidScore(score.local) &&
                      isValidScore(score.visitante) &&
                      parseInt(String(score.local)) ===
                        parseInt(String(score.visitante));

                    // Resolvemos quiénes juegan basándonos en la lógica oficial
                    const cruceResuelto =
                      m.id > 72
                        ? llavesOficialesAdmin.find((lx) => lx.id === m.id)
                        : null;

                    // idLocal e idVisitante ahora son dinámicos
                    const idLocal = cruceResuelto
                      ? cruceResuelto.equipoLocal
                      : m.equipoLocal;
                    const idVisitante = cruceResuelto
                      ? cruceResuelto.equipoVisitante
                      : m.equipoVisitante;

                    const localTeam = EQUIPOS.find((e) => e.id === idLocal);
                    const visitanteTeam = EQUIPOS.find(
                      (e) => e.id === idVisitante
                    );

                    const pensValid = checkPensValidity(
                      score.penalesLocal,
                      score.penalesVisitante
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 font-black uppercase text-zinc-500 text-[10px] tracking-tight">
                          {m.fase}
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center justify-center gap-6">
                            <div
                              className="flex items-center gap-3 justify-end w-48"
                              title={m.placeholderLocal}
                            >
                              <span
                                className={`text-[10px] font-black uppercase text-right truncate ${
                                  localTeam
                                    ? "text-zinc-200"
                            ...
                            <span className="text-zinc-800 text-[8px] font-black">
                              VS
                            </span>

                            <div
                              className="flex items-center gap-3 justify-start w-48"
                              title={m.placeholderVisitante}
                            >
                              <div className="w-8 h-5 rounded-sm overflow-hidden border border-zinc-800 shrink-0 bg-zinc-900 shadow-sm">
                                <img
                                  src={getFlagUrl(idVisitante)}
                                  className="w-full h-full object-cover"
                                  alt=""
                                />
                              </div>
                              ...
                              >
                                {visitanteTeam?.nombre ||
                                  m.placeholderVisitante}
                              </span>
                            </div>
                          </div>
                          {m.estadio && (
                            <div className="text-center mt-1">
                              <span className="text-[7px] text-zinc-700 font-bold uppercase tracking-widest">
                                {m.estadio}
                              </span>
                            </div>
                          )}
                          ...
                          <div className="flex flex-col items-center gap-2">
                            <div className="flex items-center gap-2 bg-zinc-950 p-1.5 rounded-xl border border-zinc-800 group-hover:border-emerald-500/30 transition-all">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  updateScore(m.id, "L", e.target.value)
                                }
                                placeholder="-"
                              ...
                              <input
                                type="text"
                                value={
                                  score.visitante === "-" ? "" : score.visitante
                                }
                                onChange={(e) =>
                                  updateScore(m.id, "V", e.target.value)
                                }
                                placeholder="-"
                                className="w-9 h-9 bg-zinc-900 border border-zinc-800 rounded-lg text-center font-black text-white text-sm outline-none focus:border-emerald-500"
                              />
                              ...
                              <div className="flex flex-col items-center gap-1 animate-in slide-in-from-top-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      updatePens(m.id, "L", e.target.value)
                                    }
                                    placeholder="P"
                                    className={`w-7 h-7 bg-zinc-950 border rounded-lg text-center text-[10px] font-black outline-none transition-all ${
                                      !pensValid
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      updatePens(m.id, "V", e.target.value)
                                    }
                                    placeholder="P"
                                    className={`w-7 h-7 bg-zinc-950 border rounded-lg text-center text-[10px] font-black outline-none transition-all ${
                                      !pensValid
          ...
          <div className="p-8 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-zinc-950 p-6 rounded-[2rem] border border-zinc-800 space-y-6">
                <h4 className="text-sm font-black uppercase text-emerald-500 italic flex items-center gap-3">
                  <ShieldAlert className="w-5 h-5" /> Fases
                </h4>
                <div className="space-y-4">
                  {[
                    {
                      id: "groupsPhaseLocked",
                      label: "Grupos",
                      current: globalSettings.groupsPhaseLocked,
                    },
                    {
                      id: "knockoutPhaseLocked",
                      label: "Playoffs",
                      current: globalSettings.knockoutPhaseLocked,
                    },
                  ].map((phase) => (
                    <div
                      key={phase.id}
                    ...
                    >
                      ...
                      >
                        {phase.current ? "Bloqueado" : "Habilitado"}
                      </button>
                    </div>
                  ))}
                </div>
        ...
        {activeSubTab === "logs" && (
          <div className="p-8 space-y-6 flex-1 flex flex-col">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-black uppercase italic text-white flex items-center gap-3">
                <History className="w-5 h-5 text-emerald-500" /> Auditoría
              </h3>
              <button
                onClick={fetchAuditLogs}
                className="p-3 bg-zinc-900 border border-zinc-800 rounded-xl text-zinc-500 hover:text-emerald-500 transition-all"
              >
            ...
            <div className="space-y-3 overflow-y-auto max-h-[500px] pr-2 scrollbar-hide">
              {auditLogs
                .map((log, i) => (
                  <div
                    key={i}
                    className="p-4 bg-zinc-950 border border-zinc-900 rounded-2xl flex items-start gap-4"
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-[9px] font-mono text-zinc-600">
                          {new Date(log[0]).toLocaleTimeString()}
                        </span>
                      </div>
                      <p className="text-[11px] text-zinc-400">
                        <span className="text-white font-black">@{log[1]}</span>
                        : {log[3]} - {log[4]}
                      </p>
                    </div>
                  </div>
