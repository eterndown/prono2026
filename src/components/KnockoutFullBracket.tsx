...
const FullBracketMatch: React.FC<{
  ...
  const handleScoreChange = (side: "local" | "visitante", value: string) => {
    if (disabled) return;
    let processed: number | string | null =
      value === ""
        ? null
        : !isNaN(Number(value))
        ? Math.min(Math.max(0, parseInt(value, 10)), 15)
        : value;
    if (side === "local")
      onUpdate(
        processed,
        pronostico.visitante,
        pronostico.penalesLocal,
  ...
  const isWon = (side: "L" | "V") => {
    if (!isNumeric(pronostico.local) || !isNumeric(pronostico.visitante))
      return false;
    ...
    if (l === v) {
      const pl = isNumeric(pronostico.penalesLocal)
        ? Number(pronostico.penalesLocal)
        : 0;
      const pv = isNumeric(pronostico.penalesVisitante)
  ...
  const isDraw =
    isNumeric(pronostico.local) &&
    isNumeric(pronostico.visitante) &&
    parseInt(String(pronostico.local), 10) ===
      parseInt(String(pronostico.visitante), 10);

  return (
    <div
      className={`w-full bg-zinc-950 border ${
        disabled ? "border-amber-500/20" : "border-zinc-800"
      } rounded-xl overflow-hidden shadow-xl transition-all relative ${
        isWon("L") || isWon("V") ? "ring-1 ring-emerald-500/30" : ""
      }`}
    >
      <div
        className={`flex items-center px-1.5 py-1 bg-zinc-900 border-b border-zinc-800/50 ${
          reverse ? "flex-row-reverse" : ""
        }`}
      >
        <span className="text-[7px] font-black text-emerald-500 uppercase tracking-widest px-1 bg-zinc-950 rounded border border-zinc-800/50">
          M#{match.id}
        </span>
      </div>
      <div className="p-0.5 space-y-0.5">
        {[
          {
            team: local,
            placeholder: match.placeholderLocal,
            side: "L" as const,
            key: "local" as const,
          },
          {
            team: visitante,
            placeholder: match.placeholderVisitante,
            side: "V" as const,
          ...
          return (
            ...
            >
              ...
              >
                ...
                <span
                  className={`text-[8px] font-black uppercase truncate tracking-tighter ${
                    won ? "text-emerald-400" : "text-zinc-200"
                  }`}
                >
              ...
              <div className="flex items-center gap-1">
                {isDraw && (
                  <input
                    ...
                    onChange={(e) =>
                      handlePensChange(
                        side === "L" ? "local" : "visitante",
                        e.target.value
                      )
                    }
                    placeholder="P"
                    className={`w-3.5 h-3.5 bg-zinc-950 border rounded text-center text-[7px] font-black outline-none transition-all ${
                      !pensValid
                        ? "border-rose-500 text-rose-500 shadow-[0_0_5px_rgba(244,63,94,0.3)]"
                ...
                <input
                  type="text"
                  disabled={disabled}
                  value={getDisplayValue(pronostico[key])}
                  onChange={(e) => handleScoreChange(key, e.target.value)}
                  className={`w-4 h-4 bg-zinc-900 border border-zinc-800 rounded text-center text-[8px] font-black outline-none transition-all ${
                    disabled
                      ? "text-zinc-700"
...
const KnockoutFullBracket: React.FC<KnockoutFullBracketProps> = ({
  llaves,
  pronosticos,
  onUpdate,
  disabled,
  ...
  const leftR32 = [74, 77, 73, 75, 83, 84, 81, 82].map(
    (id) => llaves.find((m) => m.id === id)!
  );
  const leftR16 = [89, 90, 93, 94].map(
    (id) => llaves.find((m) => m.id === id)!
  );
  const leftQF = [97, 98].map((id) => llaves.find((m) => m.id === id)!);
  const leftSF = llaves.find((m) => m.id === 101)!;
  const rightR32 = [76, 78, 79, 80, 86, 88, 85, 87].map(
    (id) => llaves.find((m) => m.id === id)!
  );
  const rightR16 = [91, 92, 95, 96].map(
    (id) => llaves.find((m) => m.id === id)!
  );
  ...
  return (
    <div
      ref={containerRef}
      className="w-full flex justify-center py-10 overflow-x-auto scrollbar-hide min-h-[800px]"
    >
      <div
      ...
      >
          ...
          <div className="flex flex-col justify-around gap-6 w-28">
            {leftR32.map((m) => (
              <FullBracketMatch
                ...
                match={m}
                disabled={disabled}
                pronostico={pronosticos[m.id] || { local: "-", visitante: "-" }}
                onUpdate={(l, v, pl, pv) => onUpdate(m.id, l, v, pl, pv)}
              />
            ))}
          </div>
        ...
        <div className="flex flex-col items-center justify-center gap-12 flex-1">
          <div className="flex items-center gap-4">
            <div className="w-28">
              <h4 className="text-[8px] font-black uppercase text-zinc-500 text-center mb-2 tracking-[0.3em]">
                Semifinal A
              </h4>
              <FullBracketMatch
                match={leftSF}
                disabled={disabled}
                pronostico={
                  pronosticos[leftSF.id] || { local: "-", visitante: "-" }
                }
                onUpdate={(l, v, pl, pv) => onUpdate(leftSF.id, l, v, pl, pv)}
              />
            </div>
            <div className="w-[180px] space-y-8">
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="bg-zinc-900 border-2 border-emerald-500/30 p-2 rounded-2xl w-full">
                  <h3 className="text-[9px] font-black uppercase text-center text-white italic tracking-[0.4em] mb-2">
                    Gran Final
                  </h3>
                  <FullBracketMatch
                    match={finalMatch}
                    disabled={disabled}
                    pronostico={
                      pronosticos[104] || { local: "-", visitante: "-" }
                    }
                    onUpdate={(l, v, pl, pv) => onUpdate(104, l, v, pl, pv)}
                  />
                </div>
              </div>
              <div className="bg-zinc-900/50 border border-zinc-800 p-2 rounded-2xl w-full">
                <h4 className="text-[7px] font-black uppercase text-center text-zinc-500 tracking-[0.2em] mb-1">
                  Tercer Puesto
                </h4>
                <FullBracketMatch
                  match={tercerPuestoMatch}
                  disabled={disabled}
                  pronostico={
                    pronosticos[103] || { local: "-", visitante: "-" }
                  }
                  onUpdate={(l, v, pl, pv) => onUpdate(103, l, v, pl, pv)}
                />
              </div>
            </div>
            <div className="w-28">
              ...
              <FullBracketMatch
                ...
                disabled={disabled}
                pronostico={
                  pronosticos[rightSF.id] || { local: "-", visitante: "-" }
                }
                onUpdate={(l, v, pl, pv) => onUpdate(rightSF.id, l, v, pl, pv)}
                reverse
              />
            </div>
          </div>
